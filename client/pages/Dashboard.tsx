...
export default function Dashboard() {
  ...
  const tableData: Student[] = useMemo(() => {
    ...
    return csvRows.map((r: any, i: number) => ({
      ...
      risk: toRisk(
        String(r.Attendance_risk ?? "Green"),
        String(r.Marks_risk ?? "Green"),
        String(r.fee_risk ?? "Green"),
      ),
      flags: [
          ...
          <Card className="p-4 shadow-sm">
            ...
            <div className="space-y-3 text-sm">
              <div className="text-muted-foreground">
                Use the controls above the table to filter by Class, Risk, and
                search by name/ID.
              </div>
              <div className="rounded-md bg-muted/50 p-3 text-xs">
                Color legend:{" "}
                <span className="ml-2 rounded-sm bg-risk-low px-1 py-0.5">
                  Low
                </span>{" "}
                <span className="ml-1 rounded-sm bg-risk-medium px-1 py-0.5">
                  Medium
                </span>{" "}
                <span className="ml-1 rounded-sm bg-risk-high px-1 py-0.5">
                  High
                </span>{" "}
                <span className="ml-1 rounded-sm bg-risk-critical px-1 py-0.5">
                  Critical
                </span>
              </div>
            </div>
        ...
        <div className="space-y-4">
            ...
            <div className="flex flex-col justify-between gap-2 sm:flex-row sm:items-center">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Identify and act on at-risk students quickly.
                </p>
              </div>
            </div>
          </div>
          <div className="rounded-lg border bg-card p-4 shadow-sm">
            <div className="mb-2 text-sm font-semibold">
              Generate report via your backend
            </div>
            <Separator className="my-3" />
          ...
          <div className="rounded-lg border bg-card p-4 shadow-sm">
            {csvRows?.length ? (
              <div className="mb-2 text-xs text-muted-foreground">
                Showing processed CSV data ({csvRows.length} students)
              </div>
            ) : null}
            <StudentTable
              data={tableData}
